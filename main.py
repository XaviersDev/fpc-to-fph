...
async def convert_plugin(request: Request):
    ...
    main_py += "    from funpaybotengine.dispatching.events import NewMessageEvent, NewSaleEvent, SaleStatusChangedEvent\n\n"
    ...
    clean_code = re.sub(r'import\s+telebot', '', clean_code)
    ...
    for func in hooks["order_status"]:
        main_py += f"@fp_router.on_sale_status_change()\nasync def fph_{func}(event: SaleStatusChangedEvent, hub: FunPayHub):\n    await {func}(hub, event)\n\n"
    ...
    for func in hooks["pre_init"]: main_py += f"        await {func}(self.hub)\n"
    if not hooks["pre_init"]: main_py += "        pass\n"

    main_py += "\n    async def post_setup(self):\n"
    for func in hooks["post_start"]: main_py += f"        await {func}(self.hub)\n"
